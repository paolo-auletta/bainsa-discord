...
| Project reconciliation | Database mutations and a generation-numbered reconciliation intent commit together. Discord channel state is applied idempotently and retried when necessary. | Never report a post-commit Discord failure as a database rollback. Suppress stale work, serialize competing workers, and do not replay one-shot history messages. |
...
| Command discovery/autocomplete | Production command registration requires visibility credentials. Autocomplete checks command channel, board tier, and university scope before any database or guild-member lookup. | Discord command visibility is defense in depth. Execution-time authorization remains authoritative and unauthorized autocomplete returns no suggestions. |
...
Only idempotent desired-state operations belong in replay. Introductory, showcase, and other
one-shot messages are best-effort and must not be duplicated by a retry.
