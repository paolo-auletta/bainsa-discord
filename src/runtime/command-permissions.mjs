  ...
  for (const command of managed) {
    ...
    try {
      ...
      if (error.message.includes('[10011]')) {
        try {
          const current = await discordApiRequest(path, { authorization: `Bearer ${accessToken}` });
...
async function discordApiRequest(path, { method = 'GET', authorization, body } = {}) {
