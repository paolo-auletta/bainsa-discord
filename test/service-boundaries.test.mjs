...
import * as governanceService from '../src/services/governance/service.mjs';
import * as projectService from '../src/services/projects/index.mjs';
...
test('repository modules stay free of Discord runtime objects and imports', async () => {
  for (const path of [
    '../src/services/governance/repository.mjs',
    '../src/services/projects/repository.mjs',
  ]) {
...
test('Discord gateway modules stay free of SQL and database clients', async () => {
  for (const path of [
    '../src/services/governance/gateway.mjs',
    '../src/services/projects/gateway.mjs',
  ]) {
    const source = await readFile(new URL(path, import.meta.url), 'utf8');
